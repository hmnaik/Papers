...
%In this paper we will focus on the 2D-3D correspondence matching $ m_i \leftrightarrow M_i $ problem. 
...
The other type of feature descriptos assume primitive features such as points, lines or conics to be present in the scene or on the target object. 
...
In Industrial scenario circles are widely present on a model, tracking target, as natural features or circular markers are attached to the model for Photogrammetric measurements \cite{luhmann_close_2006} as shown in \fref{fig:Model}.
...
%\end{tabular}
\caption{Introduction to correspondence matching problem}
...
Further, Forsyth \etal \cite{forsyth_91} proposed that for a given pair of non-coplanar circles in 3D, angle between the circle planes and distance between their centre positions are invariant quantities.
...
\par 
We propose using the aforementioned Euclidean invariants to solve 2D-3D correspondence problem when multiple non-coplanar circular features exist on a model.
...
Additionally, Real 3D models with circular markers shown in \fref{fig:Model} are used to evaluate the matching method.
